  ...
  return (
    ...
    >
      ...
      <div
        ...
        gap-4 sm:gap-8 transition-all duration-300 z-50
        ${location.pathname === '/' ? 'bg-light' : 'bg-white'} 
        ${open ? 'max-sm:translate-x-0' : 'max-sm:translate-x-full'}
        max-sm:fixed max-sm:h-screen max-sm:w-full max-sm:top-16 
        max-sm:border-t border-borderColor max-sm:p-4 right-0`}
      >
        ...
        <div className='flex max-sm:flex-col items-start sm:items-center gap-6'>
          ...
          <button onClick={()=>setShowLogin(true)} className="cursor-pointer px-8 py-2 bg-primary hover:bg-primary-dull transition-all text-white rounded-1g">Login</ button>
        </div>
      </div>
